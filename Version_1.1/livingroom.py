...
study_choreID = [80, 24, 25, 21, 76, 26, 27, 72, 93, 92, 91, 22, 20, 17]
	...
	for current_choreID in study_choreID:
		...
		if match:
			...
			if delta <= 0:
				...
				chore_colour = "(RED)"
			elif delta < 4 and study_choreID.index(current_choreID) > 5:
...
try:
	while True:
		...
		if not is_nighttime():
			colours = []
			grocy_update()
			set_wled_segments(study_choreID, colours, 150)
			print("---------------------------------------------------------------")
